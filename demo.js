...
const Book_Appointment = () => {
  ...
  const createTimeslots = () => {
    ...
    // Log the matching block if found
    if (matchingAppointment) {
      // Log each matching block
      matchingAppointment.forEach(matchingAppointment => {
        console.log('Matching Booked Appointment:', matchingAppointment);
      });
      return; // Exit the function to avoid creating time slots
    }
    ...
    if (!timeOffData.some(item => String(item.start).toUpperCase() === String(selectedSlotDuration).toUpperCase())) {
      ...
      selectedServiceData.forEach(item => {
        ...
        const { morningSlots, afternoonSlots, eveningSlots } = generateTimeSlots(fromTime, toTime, selectedSlotDuration, timeOffData);
  ...
  const generateTimeSlots = (startTime, endTime, duration, timeOffData) => {
    const morningSlots = [];
  ...
  const fetchApiContent = async () => {
    try {
      ...
      const bookedResponse = await fetch('https://retoolapi.dev/Cy0wpV/appdummy'); //BOOKED APPOINTMENT
...
//replace morning with morningDropdown
//replace day with afternoonDropdown
//replace night with eveningDropdown